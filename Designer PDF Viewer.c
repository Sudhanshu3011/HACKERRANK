 ...
 *  2. STRING word
 */
int Check(char ch, int h_count)
{
    int max = 0;

    for (int j = 0; j < h_count; j++)
    {
        char c = 'a' + j; // Use ASCII values to determine the character
        if (ch == c)
        {
            max = j;
            break; // No need to continue once the match is found
        }
    }
